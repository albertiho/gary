...
POWERBURST_COOLDOWN = datetime.datetime.now() - datetime.timedelta(hours=1)
...
def find_and_enter_altar():
    ...
    first_rotate = now + datetime.timedelta(seconds=10)
    second_rotate = now + datetime.timedelta(seconds=15)
    restart = now + datetime.timedelta(seconds=20)
    rift_x, rift_y = None, None
    while None in [rift_x, rift_y]:
        print("searching for rift")
        ...
        if now > first_rotate:
            rotate_camera(100)
            first_rotate = now + datetime.timedelta(minutes=1)

        if now > second_rotate:
            rotate_camera(-200)
            second_rotate = now + datetime.timedelta(minutes=1)
...
def craft_runes():
    global POWERBURST_COOLDOWN
    global RC_POT_ACTIVE
    global ALTAR_LAST_POSITION
    ...
    if POWERBURST_COOLDOWN < now:
        pyautogui.press(KEYBIND_FOR_POWERBURST, presses=random.randint(1, 4), interval=random.randint(20, 40) / 100)
        POWERBURST_COOLDOWN = now + datetime.timedelta(seconds=90)

    print("searching for altar")
    altar_search_timer = datetime.datetime.now() + datetime.timedelta(seconds=10)
    ...
    while None in [altar_x, altar_y]:
        ...
        if None not in [altar_x, altar_y]:
            ALTAR_LAST_POSITION = [altar_x, altar_y]

        if datetime.datetime.now() > altar_search_timer:
            perform_move(ALTAR_LAST_POSITION[0], ALTAR_LAST_POSITION[1])
            perform_click()
            time.sleep(3)
            return
...
def find_banker(use_last_position: bool = False):
    ...
    while banking_icon is None:
        ...
        if banking_icon is None:
            banker_region = pyautogui.locateOnScreen(BANKER_IMAGE, confidence=0.3, region=current_region)
...
def load_preset():
    pyautogui.press(KEYBIND_FOR_FIRST_PRESET, presses=random.randint(2, 4), interval=(random.randint(5, 10) / 100))
